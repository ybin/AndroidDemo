...
int svc_can_register(unsigned uid, uint16_t *name)
{
	...
	// root(uid==0)可以直接运行，这就是为啥我们的demo native service
	// 可以注册的原因。
...
{
    struct svcinfo *next;
	/* service对象的指针，注册的时候是这样的:
	
           defaultServiceManager()->addService(String16("Calc"), new Calc());
           
       所以这里的ptr指的就是这里的new Calc()对象。
	 */
    void *ptr; // service对象的弱引用对象指针(RefBase::weakref_type*)
...
// 处理业务逻辑，即service manager的业务实现都在这里
