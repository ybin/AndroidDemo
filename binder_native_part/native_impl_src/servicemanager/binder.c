...
struct binder_state *binder_open(unsigned mapsize)
{
    ...
    bs->mapsize = mapsize;
	// 从binder driver mmap来的内存是"只读"的，transaction的发起端准备好数据(位于发起端
	// 用户虚拟内存)，driver会负责将数据复制到接收端进程的内核虚拟内存中，而接受端内核
	// 虚拟内存跟接受端进程用户虚拟内存映射的是同一块物理内存，于是driver复制完之后，
	// 接收端"自动"(driver要将数据内存地址从内核虚拟内存转换为用户虚拟内存，只是一个数字
	// 的转换而已)就能看到这些数据，整个过程其实只有一次数据拷贝。
