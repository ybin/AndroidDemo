...
void IPCThreadState::joinThreadPool(bool isMain)
{
	...
	// ECONNREFUSED: 说明该进程在kernel中对应的process数据已经被销毁，
	// 该进程的所以线程池线程均应退出，详细说明见binder.c(driver)。
...
// transact的步骤很简单: writeTransactionData(), then waitForResponse(), that's ALL.
...
/*
incStrongHandle(), decStrongHandle(), incWeakHandle(), decWeakHandle()，
用于对象什么周期管理方面的，对应到RefBase里面的strong, weak pointer，
试想: BpBinder正在使用某一个service，那它首先就是要增减这个service对象
的strong pointer，但是由于是跨进程，所以只好由driver进行中转操作。
 */
...
// 下面两个函数: 注册、取消 death notification
...
// acquireResult 记录返回码
// 所谓的"跨进程调用是同步的"指的就是这里(while 1)，跟driver打交道(ioctl)是立刻就完成的，
// 它不会等待什么返回结果，它压根儿不知道还有什么返回结果。
//   ** 只有client会用到这个函数 **
status_t IPCThreadState::waitForResponse(Parcel *reply, status_t *acquireResult)
{
    ...
    while (1) {
        ...
        if (mIn.dataAvail() == 0) continue; // 循环等待driver的结果
        ...
        case BR_REPLY:
            {
                ...
                if (reply) {
                    if ((tr.flags & TF_STATUS_CODE) == 0) {
                    	...
                    	// 什么，没有数据只有一个status code而已，好吧，取出状态码，请driver清理数据吧
                	...
                	// 纳尼，我根本没想要什么reply啊，好吧，任务完成，请driver清理数据吧。
                    freeBuffer(NULL,
                        reinterpret_cast<const uint8_t*>(tr.data.ptr.buffer),
                        tr.data_size,
                        reinterpret_cast<const size_t*>(tr.data.ptr.offsets),
...
// 这个函数，client, server都可能执行，小心理解，注意~~~~~~坑!
status_t IPCThreadState::executeCommand(int32_t cmd)
{
    ...
    case BR_ACQUIRE:
		// 这显然是server执行的，只有server才有BBinder对象啊
    ...
    case BR_RELEASE:
		...
		// 把对象放到pending池中，下一个循环(见joinThreadPool函数)会处理它的
